...
export class PlaylistDataSenders{
    ...
    static create_body(playlist_name, description, song_ids){
        if (!playlist_name || playlist_name.trim() === "") {
            throw new Error("❌ Playlist name cannot be empty or null.");
        }
        if (!description) {
            description = "Generated by Apple Music Playlist Generator"; // Provide a default
        }
        console.log("📌 Creating Playlist Body - Name:", playlist_name, " Description:", description);
        return {
            attributes: {
                "name": playlist_name,
